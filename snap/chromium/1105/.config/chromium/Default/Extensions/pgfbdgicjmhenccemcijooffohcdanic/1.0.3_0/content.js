...
window.addEventListener("message", function (event) {
    if (event.source === window && event.data.sender === "bankid.crypto.plugin.js") {
        try {
            var command = document.getElementById("CryptoPluginInstance").value;
            document.getElementById("CryptoPluginInstance").value = "";
            ...
            console.error("EXTENSION_ERROR", e);
        } finally {
            document.getElementById("CryptoPluginInstance").value = "";
        }
    }
...
chrome.extension.onMessage.addListener(function (request) {
    request.sender = "bankid.crypto.plugin.native";
